    ...
    internal class ImportData
    {
        ...
        private static void ImportMediaFiles(Game game, string metadataPath)
        {
            ...
            try
            {
                ...
                // Import Manual
                var manualFile = Directory.EnumerateFiles(metadataDir, "Manual*").FirstOrDefault();
                if (!string.IsNullOrEmpty(manualFile) && File.Exists(manualFile))
                {
                    game.Manual = manualFile;
                    logger.Info($"Imported manual for game '{game.Name}'");
                }
        ...
        private static void ImportMediaElements(Game game, string importDir)
        {
            if (game == null || string.IsNullOrWhiteSpace(importDir) || !Directory.Exists(importDir))
            {
                return;
            }
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(iconFile))
                {
                    try
                    {
                        game.Icon = API.Instance.Database.AddFile(iconFile, game.Id);
                        logger.Info($"Imported icon for game '{game.Name}' from: {iconFile}");
                    }
                    catch (Exception ex)
                    {
                        logger.Error(ex, $"Failed to import icon for game '{game.Name}'");
                    }
                }
                ...
                if (!string.IsNullOrEmpty(coverFile))
                {
                    try
                    {
                        game.CoverImage = API.Instance.Database.AddFile(coverFile, game.Id);
                        logger.Info($"Imported cover image for game '{game.Name}' from: {coverFile}");
                    }
                    catch (Exception ex)
                    {
                        logger.Error(ex, $"Failed to import cover image for game '{game.Name}'");
                    }
                }
                ...
                // Background
                // -------------------------
                var backgroundFile = Directory
                    .GetFiles(importDir, "Background.*")
                    .FirstOrDefault();
                ...
                if (!string.IsNullOrEmpty(backgroundFile))
                {
                    try
                    {
                        game.BackgroundImage = API.Instance.Database.AddFile(backgroundFile, game.Id);
                        logger.Info($"Imported background image for game '{game.Name}' from: {backgroundFile}");
                    }
                    catch (Exception ex)
                    {
                        logger.Error(ex, $"Failed to import background image for game '{game.Name}'");
                    }
                }

                // -------------------------
                // Logo (ExtraMetadataLoader)
                // -------------------------
                var logoFile = Path.Combine(importDir, "Logo.png");
                ...
                if (File.Exists(logoFile))
                {
                    try
                    {
                        ...
                        var logoDest = Path.Combine(logoDir, "Logo.png");

                        File.Copy(logoFile, logoDest, true);

                        logger.Info($"Imported logo for game '{game.Name}' from: {logoFile}");
                    }
                    catch (Exception ex)
                    {
                        logger.Error(ex, $"Failed to import logo for game '{game.Name}'");
                ...
                // Manual
                // -------------------------
                var manualFile = Directory
                    .GetFiles(importDir, "Manual.*")
                    .FirstOrDefault();

                if (!string.IsNullOrEmpty(manualFile))
                {
                    try
                    {
                        game.Manual = manualFile;

                        logger.Info($"Imported manual for game '{game.Name}' from: {manualFile}");
                    }
                    catch (Exception ex)
                    {
                        logger.Error(ex, $"Failed to import manual for game '{game.Name}'");
                    }
                }
            ...
            {
                logger.Error(ex, $"Failed to import media elements for game '{game?.Name}'");
            }
        }
